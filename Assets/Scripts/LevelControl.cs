...
{  
  ...
  public struct CreationInfo {
    ...
    public void loadLevelData(TextAsset level_data_text) {
        ...
        foreach(var line in lines) { //행이 빈줄이면
            if(line =="") {
            ...
            foreach(var word in words) {
                ...
                //n값을 0, 1, 2,...7로 변화시켜 감으로써 8 항목을 처리한다
            ...
            if(n >= 8) {        //8항목(이상)이 제대로 처리되었다면
                //List 구조의 level_data에 level_data를 추가한다
...
/*
    private void update_level(ref CreationInfo current, CreationInfo previous) {
        switch(previous_block.block_type) {
...
*/
    ...
    //public void update() {                                  //소문자 u인것에 유의. 독자적으로 실행할것이 아님!
    public void update(float passage_time) {
        ...
        if(current_block.current_count >= current_block.max_count) {
            ...
            update_level(ref this.next_block, this.current_block, passage_time);
        }

        block_count++;      //블록의 총 수를 증가
    //}
    }
